---
title: "03.1 • Componentes Disponíveis na Instalação do Cluster ODP/Ambari"
weight: 40
---
...
Notas:
- Fonte de versões: [ODP-VDF.xml](/docs/assets/odp-vdf/ODP-VDF.xml) (ODP-1.2.2.0-128). Componentes Ambari/Utils (Infra Solr, Ambari Metrics, Log Search, Hue) não constam no VDF e têm versões conforme repositórios Ambari/Utils disponibilizados.

---

## Componentes efetivamente utilizados no cluster 

A tabela abaixo lista os serviços que foram efetivamente provisionados no ambiente `cdp.dev.br`, conforme o blueprint do projeto.
...
- Exceção: HDFS, ZooKeeper e YARN + MapReduce2 podem ser selecionados e instalados juntos no setup inicial do wizard (documento [Configuração do Ambari pela interface](../03.1-configuracao-servicos/)). Essa base foi a adotada no blueprint do ambiente `cdp.dev.br`.
...
- Garanta portas e firewall liberados conforme [Pré-requisitos](../00-pr%C3%A9requisitos/).
- Se SELinux estiver enforcing, revise políticas; recomendamos permissive durante a instalação inicial (ver [Pré-requisitos](../00-pr%C3%A9requisitos/)).
